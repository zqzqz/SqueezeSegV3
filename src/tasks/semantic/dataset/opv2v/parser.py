...
class Parser():
  ...
  def __init__(self,
    ...
    if self.train_sequences:
        self.train_dataset = OPV2V(root=self.root,
    ...
    if self.valid_sequences:
        self.valid_dataset = OPV2V(root=self.root,
    ...
    if self.test_sequences:
      self.test_dataset = OPV2V(root=self.root,
  ...
  def to_original(self, label):
    ...
    return OPV2V.map(label, self.learning_map_inv)

  def to_xentropy(self, label):
    # put label in xentropy values
    return OPV2V.map(label, self.learning_map)

  def to_color(self, label):
    # put label in original values
    label = OPV2V.map(label, self.learning_map_inv)
    # put label in color
    return OPV2V.map(label, self.color_map)
